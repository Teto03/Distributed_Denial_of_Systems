...
\item \textbf{\texttt{ElectionAck} carries no correlation id.} The handler cancels the current timer without checking who sent the ack. A late ack from an already skipped successor can therefore cancel a timer just reset for another once; the worst effect is a slower round, covered by the \texttt{GlobalElectionTimeout}.
